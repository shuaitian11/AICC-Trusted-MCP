...
def main():
    ...
    if quote_data is None:
        ...
        print(f"✓ successfully load Quote file: {args.quote}")
        print(f"  quote size: {len(quote_data)} bytes")
    
    if tenant_policy is None :
        print("\n✗ cannot load tenant policy file")
        return 1
    else:
        print(f"✓ successfully load policy files: {args.tenant}")
        print(f"  tenant-policy size: {len(tenant_policy)} bytes")

    if platform_policy is None:
        print("\n✗ cannot load plat policy file")
    else:
        print(f"✓ successfully load policy files: {args.platform}")
        print(f"  platform-policy size: {len(platform_policy)} bytes")
    if pub_key is None:
        print("\n✗ cannot load public key")
    else:
        print(f"✓ successfully load public key: {args.pubkey}")   
